...
def accuracy(typed, reference):
    ...
    if len(typed_words) <= len(reference_words):
        ...
        k = 0
        while k < len(reference_words):
            if typed_words[k] == reference_words[k]:
                corrent += 1
            all += 1
            k += 1
        all += len(typed_words) - len(reference_words)
    return corrent * 100 / all
    # END PROBLEM 3
...
def report_progress(typed, prompt, id, send):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    corrent, all = 0, len(prompt)
    for x, y in zip(typed, prompt):
        if x == y:
            corrent += 1
        else:
            break
    progress = corrent / all
...
def time_per_word(times_per_player, words):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    for time_one in times_per_player:
        time = []
        for i in range(len(time_one) - 1):
            time.append(time_one[i + 1] - time_one[i])
        times.append(time)
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fast_list = [[] for i in players]
    for i in words:
        index = min(players, key=lambda x : time(game, x, i))
        fast_list[index].append(word_at(game, i))
    return fast_list
