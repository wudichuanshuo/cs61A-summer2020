...
def balanced(m):
    ...
    if is_planet(m):
        ...
        assert is_mobile(m), "must get total weight of a mobile or a planet"
        return (length(left(m)) * total_weight(left(m)) == length(right(m)) * total_weight(right(m))) and balanced(end(left(m))) and balanced(end(right(m)))
...
def quadratic(x, a, b, c):
    ...
    if t > lower_bound(x) and t < upper_bound(x):
        small_value = min(left_value, right_value, special_value)
        big_value = max(left_value, right_value, special_value)
    else:
        small_value = min(left_value, right_value)
        big_value = max(left_value, right_value)

    return interval(small_value, big_value)
