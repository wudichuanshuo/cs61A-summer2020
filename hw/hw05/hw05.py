...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, type_password):
        nonlocal locked, balance
        if locked:
            return f"Too many incorrect attempts. Attempts: {incorrent_passwords}"
        elif type_password == password:
            if amount <= balance:
                balance -= amount
                return balance
            else:
            ...
            incorrent_passwords.append(type_password)
            if len(incorrent_passwords) == 3:
                locked = True
