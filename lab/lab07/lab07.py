...
def make_glookup(class_assignments):
    ...
    "*** YOUR CODE HERE ***"
    dict1 = {}
    def student(assignment, score):
        dict1[assignment] = score
        return sum(dict1.values()) / sum([class_assignments[key] for key in dict1])
    return student
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        local_number = 0
        def counter(message):
            nonlocal global_number, local_number
            if message == 'count':
                local_number += 1
                return local_number
